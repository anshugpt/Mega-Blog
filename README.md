...
- Important to add __register__ into input component.
- It required a __required, validator and many more__.
- Now we gonna make a signUp component.
```javascript JSX
import React, {useState} from 'react'
import authService from '../appwrite/auth'
import {Link ,useNavigate} from 'react-router-dom'
...
```
- Now we gonna make __authLayout__ to protect our component.
```javascript JSX
...
```
- Now we gonna add RTE file in component :-
```javascript JSX
import React from 'react'
...
```
- Now we gonna add post form file in  -> post-form folder -> components
```javascript JSX
